...
function preliminaryNodeAPIPipeline(options) {
  return (0, import_function.pipe)(
    TE.tryCatch(
      () => (0, import_resolveBinary.resolveBinary)(import_fetch_engine.BinaryType.libqueryEngine, options.prismaPath),
      (e) => ({
    ...
    TE.chainW((queryEnginePath) => {
      return (0, import_function.pipe)(
        TE.tryCatch(import_get_platform.isNodeAPISupported, (e) => ({
...
function preliminaryBinaryPipeline(options) {
  return (0, import_function.pipe)(
    TE.tryCatch(
      () => (0, import_resolveBinary.resolveBinary)(import_fetch_engine.BinaryType.queryEngine, options.prismaPath),
      (e) => ({
        type: "query-engine-unresolved",
        reason: "Unable to resolve path to query-engine binary",
        error: e
    ...
    TE.chainW(({ queryEnginePath }) => {
      if (!options.datamodelPath) {
        return (0, import_function.pipe)(
          TE.tryCatch(
            () => (0, import_temp_write.default)(options.datamodel),
            (e) => ({
...
function loadNodeAPILibrary(queryEnginePath) {
  return (0, import_function.pipe)(
    E.tryCatch(
      () => (0, import_load.load)(queryEnginePath),
      (e) => {
        const error = e;
        const defaultErrorMessage = `Unable to establish a connection to query-engine-node-api library.`;
        const proposedErrorFixMessage = (0, import_ts_pattern.match)(error.message).when(
          ...
          () => {
            ...
            return ` It seems that the current architecture ${import_chalk.default.redBright(
              architecture
            )} is not supported, or that ${import_chalk.default.redBright("libc")} is missing from the system.`;
          }
        ).otherwise(() => "");
...
function createSchemaValidationError(reason) {
  return `${import_chalk.default.redBright.bold("Schema validation error")} - ${reason}`;
}
