...
async function getPackedPackage(name, target, packageDir) {
  ...
  const tmpDir = target != null ? target : import_tempy.default.directory();
  const pkgJson = require(`${packageDir}/package.json`);
  const pkgFiles = ((_a = pkgJson.files) != null ? _a : []).concat(["package.json"]).map((file) => import_path.default.join(packageDir, file));
  for (const file of [...pkgFiles]) {
    const dest = import_path.default.join(tmpDir, import_path.default.basename(file));
    (0, import_fs_extra.copySync)(file, dest, { recursive: true, overwrite: true });
  }
