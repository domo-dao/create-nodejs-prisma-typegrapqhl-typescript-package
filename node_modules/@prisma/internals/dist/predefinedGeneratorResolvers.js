...
const predefinedGeneratorResolvers = {
  ...
  1. Rename ${import_chalk.default.red('provider = "photonjs"')} to ${import_chalk.default.green(
      'provider = "prisma-client-js"'
    )} in your ${import_chalk.default.bold("schema.prisma")} file.
  2. Replace your ${import_chalk.default.bold("package.json")}'s ${import_chalk.default.red("@prisma/photon")} dependency to ${import_chalk.default.green(
      "@prisma/client"
    )}
  3. Replace ${import_chalk.default.red("import { Photon } from '@prisma/photon'")} with ${import_chalk.default.green(
      ...
      if (process.platform === "win32" && isYarnUsed(baseDir)) {
        ...
        throw new Error(
          `Could not resolve ${missingCli(`${import_chalk.default.bold("prisma")} and `)}${import_chalk.default.bold(
            ...
            `${import_chalk.default.bold.greenBright(`${getAddPackageCommandName(baseDir, "dev")} prisma`)} and `
          )}${import_chalk.default.bold.greenBright(`${getAddPackageCommandName(baseDir)} @prisma/client`)}, and rerun ${import_chalk.default.bold(
            (0, import_getCommandWithExecutor.getCommandWithExecutor)("prisma generate")
          )} \u{1F64F}.`
        );
      }
      if (!prismaCliDir) {
...
Please try to install it by hand with ${import_chalk.default.bold.greenBright(
          ...
          )} and rerun ${import_chalk.default.bold((0, import_getCommandWithExecutor.getCommandWithExecutor)("prisma generate"))} \u{1F64F}.`
        );
      }
      console.info(
        `
      ...
      if (agent === "yarn" && major === 1) {
        ...
        if (semverLt(currentYarnVersion, minYarnVersion)) {
          import__.logger.warn(
            `Your ${import_chalk.default.bold(
              "yarn"
            )} has version ${currentYarnVersion}, which is outdated. Please update it to ${import_chalk.default.bold(
              minYarnVersion
            )} or ${import_chalk.default.bold("newer")} in order to use Prisma.`
          );
        }
      }
    }
  ...
  try {
    ...
    if (typescriptPkg && import_fs.default.existsSync(typescriptPkg)) {
      ...
      if (semverLt(currentVersion, minVersion)) {
        import__.logger.warn(
          `Prisma detected that your ${import_chalk.default.bold(
            "TypeScript"
          )} version ${currentVersion} is outdated. If you want to use Prisma Client with TypeScript please update it to version ${import_chalk.default.bold(
            minVersion
          )} or ${import_chalk.default.bold("newer")}. ${import_chalk.default.dim(`TypeScript found in: ${import_chalk.default.bold(typescriptPath)}`)}`
        );
      }
    }
  } catch (e) {
